...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-4 font-mono">
      <div className="max-w-4xl w-full bg-white border border-slate-200 rounded-3xl p-10 shadow-[0_0_50px_-15px_rgba(0,0,0,0.1)] relative overflow-hidden">
        
        <div className="mb-10 flex justify-between items-end border-b border-slate-100 pb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-[#55B82F] uppercase italic">Реєстрація системи</h1>
            <p className="text-[#55B82F] text-[10px] font-black tracking-[0.3em] mt-1 uppercase">Ідентифікація персоналу</p>
          </div>
          <div className="text-right">
            <span className="text-slate-400 text-[10px] uppercase font-black tracking-widest block mb-1">Прогрес</span>
            <div className="flex gap-1">
                {[1, 2, 3].map((s) => (
                    <div key={s} className={`h-1.5 w-8 rounded-full transition-all duration-500 ${s <= step ? 'bg-[#55B82F]' : 'bg-slate-200'}`} />
                ))}
            </div>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-2xl flex items-start gap-3 text-red-600 text-[10px] font-black uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="md:col-span-2">
              <InputField 
                ...
                error={touchedFields.includes("email") && !formData.email}
            ...
            <button 
              ...
              className="md:col-span-2 mt-4 p-5 bg-[#55B82F] text-white rounded-2xl font-black uppercase hover:bg-[#4a9f28] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#55B82F]/20"
            >
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                  <p className="text-[9px] text-[#888888] font-black uppercase tracking-widest mb-2">Вимоги:</p>
              ...
              <div className="space-y-4">
                ...
                {formData.confirmPassword && (
                  <div className={`p-4 rounded-2xl border flex items-center gap-3 transition-all ${passwordsMatch ? 'bg-green-50 border-green-100 text-[#55B82F]' : 'bg-red-50 border-red-100 text-red-600'}`}>
            ...
            <div className="flex gap-4 pt-4">
              <button type="button" onClick={() => setStep(1)} className="flex-1 p-5 border border-slate-200 rounded-2xl uppercase font-black text-[11px] hover:bg-slate-50 transition-all flex items-center justify-center gap-2 text-slate-500">
                <ArrowLeft size={16} /> Назад
              </button>
              <button type="submit" disabled={loading} className="flex-[2] p-5 rounded-2xl font-black uppercase text-[11px] transition-all flex items-center justify-center gap-2 bg-[#55B82F] text-white hover:bg-[#4a9f28] shadow-lg shadow-[#55B82F]/20 disabled:opacity-50">
        ...
        {step === 3 && (
          <div className="text-center py-6 animate-in zoom-in-95 duration-500">
             <div className="inline-flex p-6 border-4 border-[#55B82F]/10 rounded-3xl mb-8">
                <QRCodeSVG value={qrCode} size={200} />
             </div>
             <h2 className="text-xl font-black text-slate-900 mb-2 uppercase italic tracking-tight">Налаштування 2FA</h2>
             <p className="text-[#888888] text-[10px] max-w-sm mx-auto mb-10 leading-relaxed uppercase font-black tracking-widest">
               Відскануйте QR-код у додатку <span className="text-[#55B82F] underline underline-offset-4">Google Authenticator</span>.
             </p>
             <button onClick={() => router.push('/auth/login')} className="w-full max-w-xs p-5 bg-[#55B82F] text-white rounded-2xl font-black uppercase text-[11px] hover:bg-[#4a9f28] transition-all shadow-lg shadow-[#55B82F]/20">
               Перейти до входу
             </button>
          </div>
        )}
...
function InputField({ label, type = "text", value, onChange, placeholder, error }: InputFieldProps) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-[9px] uppercase tracking-[0.25em] text-[#888888] ml-1 font-black">{label}</label>
      <input 
        type={type} 
        value={value} 
        placeholder={placeholder}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange(e.target.value)}
        className={`bg-white border-2 p-4 rounded-2xl text-slate-900 focus:outline-none transition-all placeholder:text-slate-200 text-sm font-bold
          ${error ? 'border-red-200 bg-red-50' : 'border-[#55B82F]/20 focus:border-[#55B82F]'}`}
      />
    </div>
  );
}
...
function ValidationRule({ label, valid }: { label: string, valid: boolean }) {
  return (
    <div className={`flex items-center gap-2 text-[10px] font-black tracking-widest transition-colors ${valid ? 'text-[#55B82F]' : 'text-slate-300'}`}>
      {valid ? <Check size={12} strokeWidth={3} /> : <div className="w-1.5 h-1.5 bg-slate-200 rounded-full ml-1" />}
      {label}
    </div>
  );
}